...
import com.bob.api.integration.facebook.builder.FacebookMessageBuilder;
...
public class FacebookWebhookProcessor {
    ...
    public void process(JsonNode payload) {
        if (payload == null || !payload.path("entry").isArray()) {
            log.info("FB_WEBHOOK action=process result=SKIPPED reason=no_entry_array");
            return;
        }
        for (JsonNode entry : payload.path("entry")) {
            logBatchSize("messaging", entry.path("messaging"));
            logBatchSize("standby", entry.path("standby"));
            processBatch(entry.path("messaging"));
            processBatch(entry.path("standby"));
        }
    }

    private void logBatchSize(String batchName, JsonNode events) {
        if (!events.isArray()) {
            return;
        }
        log.info("FB_WEBHOOK action=process batch={} eventCount={}", batchName, events.size());
    }
    ...
    private void processBatch(JsonNode events) {
        ...
        for (JsonNode raw : events) {
            ...
            log.info("FB_WEBHOOK action=process_event senderId={} recipientId={} echo={} mid={}",
                    event.senderId(), event.recipientId(), event.echo(), event.identifier());
            if (event.deliveryOrRead()) {
                log.info("FB_WEBHOOK action=process_event result=SKIPPED reason=delivery_or_read");
                continue;
            }
            try {
                if (event.echo()) {
