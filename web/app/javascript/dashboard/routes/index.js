...
const redirectTo = (to, next, path) => {
  if (normalizePath(to.path) === normalizePath(path)) {
    return next();
  }
  return next(path);
};
...
export const validateAuthenticateRoutePermission = async (to, next) => {
  ...
  if (!accounts.length) {
    ...
    return redirectTo(to, next, frontendURL('no-accounts'));
  }
  ...
  if (to.name === 'no_accounts' || !to.name) {
    ...
    return redirectTo(
      to,
      next,
      frontendURL(`accounts/${routeAccountId}/${target}`)
    );
  }
  ...
  if (!needsOnboarding && isOnOnboardingView(to)) {
    return redirectTo(
      to,
      next,
      frontendURL(`accounts/${routeAccountId}/dashboard`)
    );
  }

  const nextRoute = validateLoggedInRoutes(to, store.getters.getCurrentUser);
  return nextRoute ? redirectTo(to, next, frontendURL(nextRoute)) : next();
};
